...
class Viewer:
    ...
    def barplot_monthly_balance(self):
        ...
        months = sorted(self.wages.keys() | self.holidays.keys()| self.house_expenses.keys() | self.fuel_expenses.keys())
        ...
        plt.bar(x , fuel_values, width=bar_width, label='Fuel Expenses', color='blue')
    ...
    def barplot_general(self):
        ...
        balances = [self.wages.get(month, 0) + self.holidays.get(month, 0) + self.house_expenses.get(month, 0) + self.fuel_expenses.get(month,0) for month in months]
